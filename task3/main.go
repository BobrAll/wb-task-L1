...
func main() {
	...
	printInstruction()
	go listenExitCommand(stopSignalCh)
...
func printInstruction() {
	fmt.Println("To start printing press 'Enter'. Than press 'Enter' again to stop printing.")
	waitForUserInput()
}
...
// listenExitCommand waits for Enter press to signal all goroutines to stop.
func listenExitCommand(stopSignalCh chan struct{}) {
	waitForUserInput()
	close(stopSignalCh)
}
